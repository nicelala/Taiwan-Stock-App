import type { RefreshLogsParams, RefreshLogsResponse } from "../types/admin";
...
export async function getRefreshLogs(
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/v1/admin/refresh/logs?${queryString}`,
    {
      method: "GET",
      headers: {
  ...
  const data = (await response.json()) as RefreshLogsResponse;
  return data;
}